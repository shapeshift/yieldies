...
describe.only("Integration", function () {
  ...
  it("Should do everything", async () => {
    ...
    warmupRewardTokenBalance = await yieldy.balanceOf(stakingWarmup.address);
    expect(warmupRewardTokenBalance).eq(89523809523810); // stakingAmount3 + stakingAmount2 + rewards
    ...
    const warmUpStaker3Reward = await yieldy.tokenBalanceForCredits(warmUpInfo.credits);
    ...
    expect(await yieldy.tokenBalanceForCredits(coolDownInfo.credits)).eq(78002322880370);
    ...
    let warmUpLP2Reward = await yieldy.tokenBalanceForCredits(warmUpInfo.credits);
